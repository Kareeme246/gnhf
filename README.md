...
| `--max-rate-limit-wait <duration>` | Abort after this much total Claude usage-limit wait (`30m`, `2h`, or `0`); implies `--usage-reset` | `0` (immediate abort)  |
...
- The TUI's model/effort line prefers `agentModel`/`agentEffort` (or `--model`/`--effort`), but when those are unset it falls back to reading a model/effort out of `agentArgsOverride.<name>` for display: `--model`/`-m` for `codex`, `--model` for `claude`/`copilot`/`pi`/`cursor`, `-c model_reasoning_effort="<value>"` for `codex`, and `--thinking <value>` for `pi`. This is display-only recovery - it never changes what `gnhf` actually passes to the agent.
